  ...
  if (Packet[3] == 0x91){                                     //If packet ID = 0x91
        ...
        if (Packet[17] == 0x00){                                        //If packet Cluster = 00.
            ...
            On_Off();                                                    //Process On/Off.
				...
				else if (Packet[17] == 0x02){                                    //Else if packet profile is not = 06.
						Serial.print("Temp ");
						Temp();                                                    //Process On/Off.
				}
        else{
...
void Temp(){
    ...
    Serial.print("Temp");
    char Command_ID = Packet[23];                                                          //Set command ID to 0.
    char Sequence_Number = 0;                                                     //Set seqence number to 0.
    char Frame_Type = Packet[21];                   //Get frame type from payload.
    Frame_Type = Frame_Type & 0x01;                                     //Bitwise & with 0x03 to make sure you are looking at the first 2 bits.
    char Attribute_ID[2] = {Packet[24],Packet[25]};
    char Cluster[2] = {Packet[16],Packet[17]};
    char Profile[2] = {Packet[18],Packet[19]};
    char Buffer[64];
    ...
    memset(Buffer, 0, sizeof(Buffer));                                  //Clear array.
//
//    if ((Frame_Type == 0x00) && (Command_ID == 0x00) && (Attribute_ID[0] == 0x00)){     //If Command to check status is sent.
//      int Water_val=0;
//      Water_val=analogRead(5) - 8; // analog pin 5
//      Temperature[0] = (Water_val >> 8) & 0xFF;
//      Temperature[1] = Water_val & 0xFF;
//      Serial.println(Water_val);
//    }
//
//    if ((Frame_Type == 0x00) && (Command_ID == 0x00) && (Attribute_ID[0] == 0x01)){     //If Command to check status is sent.
//      int Water_val=0;
//      Water_val=analogRead(5) - 8; // analog pin 5
//      Temperature[0] = (Water_val >> 8) & 0xFF;
//      Temperature[1] = Water_val & 0xFF;
...
//    }
//
//    if ((Frame_Type == 0x00) && (Command_ID == 0x00) && (Attribute_ID[0] == 0x02)){     //If Command to check status is sent.
//      int Water_val=0;
//      Water_val=analogRead(5) - 8; // analog pin 5
//      Temperature[0] = (Water_val >> 8) & 0xFF;
//      Temperature[1] = Water_val & 0xFF;
...
//    }
      ...
      int Water_val=0;
      Water_val=analogRead(5); // analog pin 5
      Temperature[0] = (Water_val >> 8) & 0xFF;
      Temperature[1] = Water_val & 0xFF;
      Serial.println(Water_val);
...
//      Serial.print(digitalRead(13));
      ...
      Buffer[1] = Sequence_Number;                                                 //Reply with seqence number from request.
      Buffer[2] = 0x01;                                                   //Command indetifier = 1, Read attribute response.
      Buffer[3] = Attribute_ID[0];                                                   //Attribute Identfier (2 bytes) field being reported.
      Buffer[4] = Attribute_ID[1];
      Buffer[5] = 0x00;                                                   //Status 00 = success.
      Buffer[6] = 0x29;                                                   //Attribute data type 10 = boolean.
      Buffer[7] = Temperature[1];
      Buffer[8] = Temperature[0];
      Buffer[9] = 0x04;  // 더미 왜 그런지 버퍼 9개 보내면 에러남 ㅋㅋ
      PBuild(ST_Address,Packet[15],Packet[14],Profile,Cluster,Buffer,10);
